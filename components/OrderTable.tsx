...
interface OrderTableProps {
  orders: (OrderTab1 | OrderTab2 | InventoryItem)[];
  type: 'tab1' | 'tab2' | 'inventory';
  onEdit: (order: OrderTab1 | OrderTab2 | InventoryItem) => void;
...
export default function OrderTable({
  ...
  if (type === 'tab1') {
                ...
                <tr key={order.id} className="hover:bg-pink-50">
                  ...
                  {visibleColumns.remaining_amount !== false && (
                    <td className="border p-2 text-sm">
                      <span className={`font-semibold ${order.remaining_amount > 0
                  ...
                  {visibleColumns.priority !== false && (
                    <td className="border p-2 text-sm">
                      <span
                        className={`px-2 py-1 rounded text-xs ${order.priority === 'Gấp'
    ...
    const inventoryOrders = orders as InventoryItem[];
    const shouldScroll = inventoryOrders.length > 10;
    return (
      <div className="table-container">
        <div className={`bg-white rounded-lg shadow ${shouldScroll ? 'max-h-[600px] overflow-y-auto' : ''} overflow-x-auto`}>
          <table className="w-full border-collapse min-w-[800px]">
            <thead className={`bg-pink-100 ${shouldScroll ? 'sticky top-0 z-10' : ''}`}>
              <tr>
                ...
                {visibleColumns.note !== false && (
                  <th className="border p-2 text-left text-xs font-semibold">Ghi chú</th>
                )}
                <th className="border p-2 text-left text-xs font-semibold">Thao tác</th>
              </tr>
            </thead>
            <tbody>
              {inventoryOrders.map((order) => (
                <tr key={order.id} className="hover:bg-pink-50">
                  ...
                  {visibleColumns.reported_amount !== false && (
                    <td className="border p-2 text-sm">{formatCurrency(order.reported_amount || 0)}</td>
                  )}
                  {visibleColumns.capital !== false && (
                    <td className="border p-2 text-sm">{formatCurrency(order.capital || 0)}</td>
                  )}
                  {visibleColumns.profit !== false && (
                    <td className="border p-2 text-sm">
                      <span
                        className={`font-semibold px-2 py-1 rounded ${order.profit >= 0
                  ...
                  {visibleColumns.shipping_fee !== false && (
                    <td className="border p-2 text-sm">{formatCurrency(order.shipping_fee || 0)}</td>
                  )}
                  {visibleColumns.domestic_shipping_fee !== false && (
                    <td className="border p-2 text-sm">{formatCurrency(order.domestic_shipping_fee || 0)}</td>
                  )}
                  {visibleColumns.status !== false && (
                    <td className="border p-2 text-sm">
                      <span className={`px-2 py-1 rounded text-xs ${order.status === 'Kho' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
                        {order.status}
                      </span>
                    </td>
                  )}
                  ...
                  {visibleColumns.note !== false && (
                    <td className="border p-2 text-sm max-w-[200px] truncate" title={order.note}>
                      {order.note}
                    </td>
                  )}
                  <td className="border p-2">
